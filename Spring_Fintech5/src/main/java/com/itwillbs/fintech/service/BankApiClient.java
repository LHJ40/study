...
//	@Autowired
	...
	public ResponseTokenVO requestToken(Map<String, String> authResponse) {
		...
		// 1. 헤더 설정(org.springframework.http.HttpHeaders 클래스 활용)
		//    - 객체 생성 후 add() 메서드를 호출하여 헤더 정보 설정
		//      => add("헤더속성명", "헤더값") 
		//    - 별도의 추가 헤더 정보가 없을 경우 객체 생성만 수행
		HttpHeaders httpHeaders = new HttpHeaders();
		
		// 2. 요청에 필요한 파라미터(Body) 설정
		...
		// 4. REST 방식 요청을 수행하기 위해 RestTemplate 객체 생성
		...
		// 6. 응답받은 ResponseEntity 객체의 getBody() 메서드를 호출하면
